...
public:
    ...
    */
    ...
    static void GetFiles(const wchar_t* path, std::function<void(const std::wstring&)> func);
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    //获取外网IP地址和IP归属地
    static void GetInternetIp(std::wstring& ip_address, std::wstring& ip_location, bool global);

    static void GetInternetIp2(std::wstring& ip_address, std::wstring& ip_location, bool ipv6);
    ...
    template<class T>
    static void RemoveVectorDuplicateItem(std::vector<T>& vec)
    {
        std::set<int> si;
