...
bool CCommon::GetFileContent(const wchar_t* file_path, std::string& contents_buff, bool binary /*= true*/)
{
...
std::wstring CCommon::GetStartUpPath()
{
    LPITEMIDLIST ppidl;
    ...
    return std::wstring(pszStartUpPath);
}
...
void CCommon::GetFiles(const wchar_t* path, std::function<void(const std::wstring&)> func)
{
    //文件句柄
    intptr_t hFile = 0;
    _wfinddata_t fileinfo;
...
std::wstring CCommon::GetModuleDir()
{
...
std::wstring CCommon::GetSystemDir()
{
...
std::wstring CCommon::GetTemplateDir()
{
    std::wstring result;
    wchar_t buff[MAX_PATH];
...
std::wstring CCommon::GetAppDataConfigDir()
{
    LPITEMIDLIST ppidl;
    ...
    std::wstring app_data_path{ pszAppDataPath };        //获取到C:/User/用户名/AppData/Roaming路径
...
std::wstring CCommon::GetJsonValueSimple(const std::wstring& json_str, const std::wstring& name)
{
    ...
    std::wstring result = json_str.substr(index, index_end - index);
    return result;
}

//bool CCommon::GetURL(const std::wstring& url, std::wstring& result, bool utf8, const std::wstring& user_agent)
//{
//    bool succeed{ false };
...
//void CCommon::GetInternetIp(std::wstring& ip_address, std::wstring& ip_location, bool global)
//{
...
//        if (index == std::wstring::npos || index1 == std::wstring::npos)
//            ip_address.clear();
//        else
...
//void CCommon::GetInternetIp2(std::wstring& ip_address, std::wstring& ip_location, bool ipv6)
//{
//    std::wstring raw_string;
//    std::wstring user_agent{ L"TrafficMonitor/" };
//    user_agent += VERSION;
